...
def parse(filename):
    try:
        ...
        try:
            ...
            table = np.zeros((1,5),dtype=float)
...
#                sparsity90 = np.append(sparsity90,np.zeros((dif,5),dtype=int),0)    
            ...
            parsed = np.zeros((1,table.shape[0],table.shape[1]),dtype=float)
            parsed[0] = np.copy(table)
            
            #vv = np.append(ff,[ones],0) #adds (2,5) array ones to (1,2,5) array ff
